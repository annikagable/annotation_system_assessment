rule calc_uniq_enriched_genes_per_term_size:
    input:
        sigTermDf_file = "data/results/cameraPR/overlap_3-200/aggregation/sigTermDf_alpha"+str(ALPHA)+".tsv", 
    ...
    output:
        ...
        summary_cum_gene_unions_file = "data/results/cameraPR/overlap_3-200/unique_enriched_genes/{taxId}/mean_median_percentiles_of_cum_gene_union_by_term_size.tsv",
        # computed only on existing term sizes => for plotting individual users curves
        cum_gene_unions_file = "data/results/cameraPR/overlap_3-200/unique_enriched_genes/{taxId}/cumulative_gene_unions_by_term_size.tsv"
    log:
        "logs/calc_uniq_enriched_genes_per_term_size/{taxId}.log"
    conda:
...
rule plot_enrichment_specificity:
    input:
        ...
        cum_gene_unions_file = "data/results/cameraPR/overlap_3-200/unique_enriched_genes/{taxId}/cumulative_gene_unions_by_term_size.tsv"
    output:
        enrichment_specificity_file = "figures/cameraPR/overlap_3-200/unique_enriched_genes/{taxId}/enrichment_specificity.svg",
        enrichment_specificity_legend_file = "figures/cameraPR/overlap_3-200/unique_enriched_genes/{taxId}/enrichment_specificity_legend.svg"
    log:
        "logs/plot_enrichment_specificity/{taxId}.log"
    conda:
        "../envs/py38_sklearn.yml"
    shell:
